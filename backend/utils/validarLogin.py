...
def validacionLogin(data):
    try:
        ...
        with conexion.cursor() as cursor:
            ...
            consulta = "SELECT id, name, email, password, rol FROM users where email = %s"      
            cursor.execute(consulta,(data['email'],))  
            usuario = cursor.fetchone()
            if 'email' not in data or 'password' not in data:
                return {"error": "Faltan campos obligatorios"}
            if usuario:
                id_usuario,user_name, user_email, password_user, rol = usuario
                ...
                if bcrypt.checkpw(password_input,password_db):
                    return {"id":id_usuario, "name" : user_name, "email": user_email, "rol": rol}
                else:
